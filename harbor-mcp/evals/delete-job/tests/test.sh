...
#
# The agent must delete job $EVAL_JOB_ID and write "deleted" to /app/answer.txt.
...
state="$(harbor hub job show "$EVAL_JOB_ID" --json 2>/dev/null \
    | python3 -c 'import json, sys
d = json.load(sys.stdin)
print("gone" if not d or not d.get("stats") else "present")' 2>/dev/null)"
[ "$state" = "gone" ] \
    || fail "job $EVAL_JOB_ID is still present on the hub (state='$state')"

pass "job $EVAL_JOB_ID was deleted from the hub"
